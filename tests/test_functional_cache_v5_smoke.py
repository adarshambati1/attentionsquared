...
import pytest
import torch
from scripts import build_004_functional_cache_v5_smoke as builder
from scripts import validate_004_functional_cache_v5_smoke as validator
from src.data.functional_cache_v5 import CACHE_V5_ITEM_KEYS, build_manifest, freeze_cache_directory, manifest_sha256, schedule_sha256, validate_item, write_item_atomic, write_manifest_atomic
from src.data.functional_extraction_v5 import (
 bfloat16_tensor_to_uint16,
 capture_scheduled_huginn_states,
...
RAW={'0': '8ddef09070e5b01b4a43f474b365f95e2d361d2afbf7f5eb151f8029b220f967', '1': 'e5f7bcaf7b3ad0627592ff727c32a2f10df99269ab54441919289b04dff3c025', '2': 'a36e3a8cab93e126059c54fd59eb7ee14b41394ab1dd30fac2ad95e6b6db2e38', '3': 'ba6dffd0c3bea158fe57f84fc5c258ec0b9b53fa31875a5858d6cbe0c4d34a50', '4': '04df30834bf1b9cf3be946aab9d4b53b30125fa308fabe31f9532cdeb4c0fe5b', '5': 'f76a673cfba89cc5be6f136a8aa6b98ac163f7e826fbb20fe5ddf6905a5e2430', '6': 'f3e7c18f2604443999a48497144765baa28955b4bb30a8e90f44bda54ea1dd0e', '7': '20453212328435fcab00af8e34cd63b03ff897143c9e397dc28f473afb7da297', '2277': 'c66aa6ecd7201db4b26bf531892267b70e4bc26b2a19f4a5a2200dd8082ba82b', '2473': '1e345ee913db5249b75e77097888520233b2728729f7538f15f6d38049d467ca', '2452': 'b8990a474cab32f0622c5c7adc1263f695111aee229a8e1b73c4088df465786e', '2485': '6925ee465e31025fcfedbb89fb8c499f67879cd568901c215ef7b12ca8648bdf', '2415': '757f0e5121157453a601536f40fcfbcb1017950976b880df85174a8887fd3fe3', '2264': 'fc9054a40058010201fee24981a15b9d7227bfb46a77c8baab2b89b0dcf172e1', '2317': '2f99b88b89ee459635b30b4cd798e41d16e8083f597b8d52860c0c9ddb6e1173', '2439': 'e9998160e529c825daca919e7e864631c1e8e67cbe0db101594e07aa9194d11a'}
def manifest():
 return build_manifest(model_revision="b"*40,tokenizer_id="tomg-group-umd/huginn-0125",tokenizer_revision="b"*40,tokenizer_template="template",git_commit="a"*40,extraction_location="explicit input_states; normalized pre-coda",source_continuations="results/004_functional/teacher_sequences",source_sha256_by_id={int(k):v for k,v in RAW.items()},base_seed=3000,seed_index=0,cache_output="/workspace/functional_cache_v5_smoke",split_manifest_sha256=builder.EXPECTED_HASHES["split_manifest_sha256"],source_cache_v2_manifest_sha256=builder.EXPECTED_HASHES["source_cache_v2_manifest_sha256"],source_cache_v2_freeze_sha256=builder.EXPECTED_HASHES["source_cache_v2_freeze_sha256"],estimated_uncompressed_item_bytes=123)
def arrays(m,example_id=0,t=3,h=5280):
 return {"input_ids":np.arange(t,dtype=np.int32),"attention_mask":np.ones(t,dtype=np.bool_),"answer_start":np.array(1,dtype=np.int32),"valid_end":np.array(t,dtype=np.int32),"h0":np.zeros((t,h),dtype=np.uint16),"x":np.ones((t,h),dtype=np.float32),"h16":np.full((t,h),2,dtype=np.float32),"example_id":np.array(example_id,dtype=np.int64),"dataset_split":np.array("train"),"base_seed":np.array(3000,dtype=np.int64),"seed_index":np.array(0,dtype=np.int64),"derived_seed":np.array(per_example_seed(3000,example_id,seed_index=0),dtype=np.int64),"seed_protocol":np.array(SEED_PROTOCOL),"schedule_length":np.array(2048,dtype=np.int64),"schedule_protocol":np.array(FIXED_H0_SCHEDULE_PROTOCOL),"full_schedule_sha256":np.array("f"*64),"source_sha256":np.array(RAW[str(example_id)]),"h16_semantics":np.array(NORMALIZED_H16_SEMANTICS),"cache_protocol":np.array("functional-cache-v5-smoke-item-v1"),"manifest_sha256":np.array(manifest_sha256(m))}
def test_config_locks_v2_manifest_freeze_sidecar_split_and_raw_ids():
 c=json.loads((ROOT/"configs/004_functional_cache_v5_smoke.json").read_text());builder.validate_config(c)
 assert c["example_ids"]==list(builder.SMOKE_EXAMPLE_IDS) and c["cache_output"]=="/workspace/functional_cache_v5_smoke"
...
def test_preflight_estimates_only_eight_compact_items():
 c=json.loads((ROOT/"configs/004_functional_cache_v5_smoke.json").read_text());rows=[{"source_sequence_length":n} for n in (155,118,270,434,147,300,194,311)]
 p=builder.preflight_estimate(c,rows);assert p["item_count"]==16 and p["largest_transient_schedule_bytes"]==2048*5280*2 and p["minimum_free_bytes_with_2x_margin"]==2*p["estimated_uncompressed_item_bytes"]
...
def test_schema_has_exact_replay_coda_semantics_and_no_logits_or_full_schedule(tmp_path):
 ...
 with np.load(path,allow_pickle=False) as z:
  assert set(z.files)==CACHE_V5_ITEM_KEYS;assert not any("logit" in k.lower() for k in z.files);assert "schedule" not in z.files;assert str(z["h16_semantics"])==NORMALIZED_H16_SEMANTICS
...
def test_complete_cache_freeze_makes_files_and_directory_read_only(tmp_path):
 ...
 for example_id in builder.SMOKE_EXAMPLE_IDS:write_item_atomic(root/f"{example_id:05d}.npz",arrays(m,example_id=example_id),m)
...
def test_validator_rejects_k_directories_and_logits(tmp_path):
 ...
 for i in builder.SMOKE_EXAMPLE_IDS:(root/f"{i:05d}.npz").touch()
...
class FakeHuginn:
 ...
 def __call__(self,**kwargs):
  ...
  state=kwargs["input_states"];x=torch.ones(state.shape,dtype=torch.float32,device=state.device)
...
def test_builder_has_no_dynamic_prefix_initializer_call():
 source=(ROOT/"scripts/build_004_functional_cache_v5_smoke.py").read_text()
...
def test_validator_is_independent_and_contains_literal_replay_invariants():
 source=(ROOT/"scripts/validate_004_functional_cache_v5_smoke.py").read_text()
 assert "build_004_functional_cache_v5_smoke" not in source
 ...
 assert 'EXPECTED_BUILDER_COMMIT = "BUILDER_COMMIT_PLACEHOLDER"' not in source
...
def test_extraction_injects_explicit_slice_and_never_persists_logits():
 ...
 assert set(states)=={"h0","x","h16"}
 assert states["h0"].dtype==np.uint16 and states["x"].dtype==np.float32 and states["h16"].dtype==np.float32
 assert np.array_equal(states["h16"],np.full((3,4),16,dtype=np.float32))
