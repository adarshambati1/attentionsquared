...
from scripts import run_004_phase12_full_prefix_evaluation_v5 as phase12


def test_phase12_config_locks_slow_full_prefix_semantics():
    root = Path(__file__).resolve().parents[1]
    config = json.loads((root / "configs/004_functional_v5_phase12.json").read_text())
    ...
    assert config["phase11_model"] == "/workspace/functional_overfit_v5_native_v2/model.pt"
    assert config["output"] == "/workspace/functional_phase12_v5_native"
    for key, value in [
        ("use_cache", True),
...
def test_phase12_runner_does_not_use_incremental_generate_or_persist_logits():
    root = Path(__file__).resolve().parents[1]
    source = (root / "scripts/run_004_phase12_full_prefix_evaluation_v5.py").read_text()
