...
import pytest
import torch
from scripts import build_004_functional_cache_v4_smoke as builder
from scripts import validate_004_functional_cache_v4_smoke as validator
from src.data.functional_cache_v4 import CACHE_V4_ITEM_KEYS, build_manifest, freeze_cache_directory, manifest_sha256, schedule_sha256, validate_item, write_item_atomic, write_manifest_atomic
...
def manifest():
 return build_manifest(model_revision="b"*40,tokenizer_id="tomg-group-umd/huginn-0125",tokenizer_revision="b"*40,tokenizer_template="template",git_commit="a"*40,extraction_location="explicit input_states; normalized pre-coda",source_continuations="results/004_functional/teacher_sequences",source_sha256_by_id={int(k):v for k,v in RAW.items()},base_seed=3000,seed_index=0,cache_output="/workspace/functional_cache_v4_smoke",split_manifest_sha256=builder.EXPECTED_HASHES["split_manifest_sha256"],source_cache_v2_manifest_sha256=builder.EXPECTED_HASHES["source_cache_v2_manifest_sha256"],source_cache_v2_freeze_sha256=builder.EXPECTED_HASHES["source_cache_v2_freeze_sha256"],estimated_uncompressed_item_bytes=123)
def arrays(m,example_id=0,t=3,h=5280):
 return {"input_ids":np.arange(t,dtype=np.int32),"attention_mask":np.ones(t,dtype=np.bool_),"answer_start":np.array(1,dtype=np.int32),"valid_end":np.array(t,dtype=np.int32),"h0":np.zeros((t,h),dtype=np.uint16),"x":np.ones((t,h),dtype=np.uint16),"h16":np.full((t,h),2,dtype=np.uint16),"example_id":np.array(example_id,dtype=np.int64),"dataset_split":np.array("train"),"base_seed":np.array(3000,dtype=np.int64),"seed_index":np.array(0,dtype=np.int64),"derived_seed":np.array(per_example_seed(3000,example_id,seed_index=0),dtype=np.int64),"seed_protocol":np.array(SEED_PROTOCOL),"schedule_length":np.array(2048,dtype=np.int64),"schedule_protocol":np.array(FIXED_H0_SCHEDULE_PROTOCOL),"full_schedule_sha256":np.array("f"*64),"source_sha256":np.array(RAW[str(example_id)]),"h16_semantics":np.array(NORMALIZED_H16_SEMANTICS),"cache_protocol":np.array("functional-cache-v4-smoke-item-v1"),"manifest_sha256":np.array(manifest_sha256(m))}
def test_config_locks_v2_manifest_freeze_sidecar_split_and_raw_ids():
 c=json.loads((ROOT/"configs/004_functional_cache_v4_smoke.json").read_text());builder.validate_config(c)
 assert c["example_ids"]==list(range(8)) and c["cache_output"]=="/workspace/functional_cache_v4_smoke"
...
def test_preflight_estimates_only_eight_compact_items():
 c=json.loads((ROOT/"configs/004_functional_cache_v4_smoke.json").read_text());rows=[{"source_sequence_length":n} for n in (155,118,270,434,147,300,194,311)]
...
def test_schema_has_exact_replay_coda_semantics_and_no_logits_or_full_schedule(tmp_path):
 ...
 with np.load(path,allow_pickle=False) as z:
  assert set(z.files)==CACHE_V4_ITEM_KEYS;assert not any("logit" in k.lower() for k in z.files);assert "schedule" not in z.files;assert str(z["h16_semantics"])==NORMALIZED_H16_SEMANTICS
...
def test_builder_has_no_dynamic_prefix_initializer_call():
 source=(ROOT/"scripts/build_004_functional_cache_v4_smoke.py").read_text()
...
def test_validator_is_independent_and_contains_literal_replay_invariants():
 source=(ROOT/"scripts/validate_004_functional_cache_v4_smoke.py").read_text()
 assert "build_004_functional_cache_v4_smoke" not in source
...
def test_extraction_injects_explicit_slice_and_never_persists_logits():
 ...
 assert all(value.dtype==np.uint16 for value in states.values())
 assert torch.equal(uint16_to_bfloat16_tensor(states["h16"]),torch.full((3,4),16,dtype=torch.bfloat16))
