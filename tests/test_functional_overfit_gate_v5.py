...
from scripts import run_004_functional_overfit_gate_v5 as runner
...
def test_v3_config_preserves_overfit_protocol_and_locks_phase10():
    config = json.loads((ROOT / "configs/004_functional_v5_overfit.json").read_text())
...
def test_runner_can_only_target_new_no_replace_v3_output():
    assert runner.CONFIG == Path("configs/004_functional_v5_overfit.json")
    assert runner.OUTPUT_ROOT == Path("/workspace/functional_overfit_v5_native")
    source = (ROOT / "scripts/run_004_functional_overfit_gate_v5.py").read_text()
...
def test_phase10_artifact_and_attestation_are_both_immutable_prerequisites(tmp_path, monkeypatch):
    ...
    artifact.write_text(json.dumps({
        "protocol": "functional-correctness-gate-cache-v5-coda-v4",
        "status": "pass",
    ...
    attestation.write_text(json.dumps({
        "protocol": "functional-correctness-gate-cache-v5-coda-v4-runtime-attestation-v1",
        "status": "pass",
...
def test_preflight_process_scan_ignores_parent_shell_text_and_matches_exact_python_script(tmp_path):
    ...
    (shell / "cmdline").write_bytes(
        b"bash\0-c\0python scripts/run_004_functional_overfit_gate_v5.py\0"
    )
    runner_process = tmp_path / "101"
    runner_process.mkdir()
    (runner_process / "cmdline").write_bytes(
        b"python\0scripts/run_004_functional_overfit_gate_v5.py\0"
    )
    ...
    assert preflight.matching_training_processes(tmp_path) == [
        {
            "pid": 101,
            "argv": ["python", "scripts/run_004_functional_overfit_gate_v5.py"],
        }
    ]
...
def test_v3_runner_and_preflight_are_unblocked_only_for_cache_v3():
    ...
    assert runner.CACHE_ROOT == Path("/workspace/functional_cache_v5_smoke")
    assert runner.PHASE10_ARTIFACT.name == "correctness_gate_cache_v3_coda_v4.json"
    runner_source = (ROOT / "scripts/run_004_functional_overfit_gate_v5.py").read_text()
