"""Crash-safe native-state schema and storage for the full Phase 14 cache."""
...
FULL_EXAMPLE_IDS = tuple(range(2500))
CACHE_V5_ITEM_PROTOCOL = "functional-cache-v5-full-item-v1"
CACHE_V5_MANIFEST_PROTOCOL = "functional-cache-v5-full-manifest-v1"
STATE_DTYPES = {"h0": np.dtype(np.uint16), "x": np.dtype(np.float32), "h16": np.dtype(np.float32)}
SHA256_PATTERN = re.compile(r"[0-9a-f]{64}")

CACHE_V5_ITEM_KEYS = frozenset(
    {
        "input_ids",
        "attention_mask",
        "answer_start",
...
CACHE_V5_MANIFEST_KEYS = frozenset(
    {
        "protocol",
        "huginn_model_id",
        "huginn_revision",
...
def build_manifest(
    *,
    ...
    manifest = {
        "protocol": CACHE_V5_MANIFEST_PROTOCOL,
        ...
        "example_ids": list(FULL_EXAMPLE_IDS),
        "sequence_count": len(FULL_EXAMPLE_IDS),
...
def validate_manifest(manifest: Mapping[str, Any]) -> None:
    if set(manifest) != CACHE_V5_MANIFEST_KEYS:
        raise ValueError("cache-v5 manifest keys do not match the required schema")
    fixed = {
        "protocol": CACHE_V5_MANIFEST_PROTOCOL,
        "huginn_model_id": HUGINN_MODEL_ID,
        "model_compute_dtype": "bfloat16-autocast; native hidden outputs float32",
    ...
    for key, expected in fixed.items():
        if manifest[key] != expected:
            raise ValueError(f"cache-v5 manifest {key} does not match the frozen full-cache protocol")
    for key in ("huginn_revision", "tokenizer_revision", "git_commit"):
        if not isinstance(manifest[key], str) or not re.fullmatch(r"[0-9a-f]{40}", manifest[key]):
            raise ValueError(f"cache-v5 manifest {key} must be a 40-character commit")
    for key in ("tokenizer_id", "extraction_location", "source_continuations"):
        if not isinstance(manifest[key], str) or not manifest[key].strip():
            raise ValueError(f"cache-v5 manifest {key} must be nonempty")
    ...
    if not isinstance(hashes, dict) or set(hashes) != {str(i) for i in FULL_EXAMPLE_IDS}:
        raise ValueError("cache-v5 manifest must contain source hashes for all 2,500 locked IDs")
    if not all(isinstance(value, str) and SHA256_PATTERN.fullmatch(value) for value in hashes.values()):
        raise ValueError("invalid source hash in cache-v5 manifest")
    if not isinstance(manifest["base_seed"], int) or manifest["base_seed"] < 0:
        raise ValueError("cache-v5 base seed must be nonnegative")
...
def validate_item(path: str | Path, manifest: Mapping[str, Any]) -> dict[str, Any]:
    ...
    try:
        with np.load(item_path, allow_pickle=False) as archive:
            if set(archive.files) != CACHE_V5_ITEM_KEYS:
                raise ValueError("item keys do not match full cache-v5 schema")
            arrays = {key: archive[key] for key in archive.files}
    except Exception as error:
        raise ValueError(f"invalid cache-v5 item {item_path}: {error}") from error
    ...
    for key, value in zip(("h0", "x", "h16"), states):
        if value.dtype != STATE_DTYPES[key]:
            raise ValueError(f"{item_path}: invalid native storage dtype for {key}")
    ...
    if example_id not in FULL_EXAMPLE_IDS:
        raise ValueError(f"{item_path}: example ID is outside full ID range 0-2499")
    expected = {
    ...
    strings = {
        ...
        "h16_semantics": NORMALIZED_H16_SEMANTICS,
        "cache_protocol": CACHE_V5_ITEM_PROTOCOL,
...
def write_item_atomic(path: str | Path, arrays: Mapping[str, Any], manifest: Mapping[str, Any]) -> str:
    ...
    with _writer_lock(final.parent):
        ...
        published = _publish_no_replace(
            ...
            redundant_reason="a matching final cache-v5 item appeared before promotion",
        )
        _require_or_make_read_only(final)
...
def write_manifest_atomic(path: str | Path, manifest: Mapping[str, Any]) -> str:
    ...
    with _writer_lock(final.parent):
        ...
        published = _publish_no_replace(
            ...
            invalid_category="invalid_concurrent_final",
            redundant_reason="a matching cache-v5 manifest appeared before promotion",
        )
        _require_or_make_read_only(final)
        return status if published else "validated_existing"
...
def freeze_cache_directory(path: str | Path) -> None:
    ...
    if directory.is_symlink() or not directory.is_dir():
        raise ValueError("full cache-v5 root must be a real directory")
    for payload in [directory / "manifest.json", *(directory / f"{i:05d}.npz" for i in FULL_EXAMPLE_IDS)]:
        if payload.is_symlink() or not payload.is_file() or stat.S_IMODE(payload.stat().st_mode) != 0o444:
            raise ValueError(f"cache-v5 payload is not immutable: {payload}")
    lock = directory / ".functional-cache-v5.lock"
    if lock.exists():
        if lock.is_symlink() or not lock.is_file():
            raise ValueError("cache-v5 writer lock is not a regular file")
        os.chmod(lock, 0o444)
...
def _validate_expected(path: Path, arrays: Mapping[str, Any], manifest: Mapping[str, Any]) -> None:
    ...
    if set(arrays) != CACHE_V5_ITEM_KEYS:
        raise ValueError("requested arrays do not match cache-v5 schema")
    with np.load(path, allow_pickle=False) as archive:
        for key in CACHE_V5_ITEM_KEYS:
...
def _writer_lock(directory: Path):
    lock_path = directory / ".functional-cache-v5.lock"
