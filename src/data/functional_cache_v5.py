"""Crash-safe schema and storage for the Phase 9R eight-item cache-v5 smoke."""
...
PHASE11_EXAMPLE_IDS = tuple(range(8))
PHASE13C_EXAMPLE_IDS = (2277, 2473, 2452, 2485, 2415, 2264, 2317, 2439)
SMOKE_EXAMPLE_IDS = PHASE11_EXAMPLE_IDS + PHASE13C_EXAMPLE_IDS
CACHE_V5_ITEM_PROTOCOL = "functional-cache-v5-smoke-item-v1"
CACHE_V5_MANIFEST_PROTOCOL = "functional-cache-v5-smoke-manifest-v1"
...
def validate_manifest(manifest: Mapping[str, Any]) -> None:
    ...
    fixed = {
        ...
        "h16_semantics": NORMALIZED_H16_SEMANTICS,
        "full_vocabulary_logits_persisted": False,
        "cache_output": "/workspace/functional_cache_v5_smoke",
    }
    for key, expected in fixed.items():
        if manifest[key] != expected:
            raise ValueError(f"cache-v5 manifest {key} does not match the frozen smoke protocol")
    ...
    if not isinstance(hashes, dict) or set(hashes) != {str(i) for i in SMOKE_EXAMPLE_IDS}:
        raise ValueError("cache-v5 manifest must contain source hashes for all 16 locked IDs")
    ...
    try:
        with np.load(item_path, allow_pickle=False) as archive:
            if set(archive.files) != CACHE_V5_ITEM_KEYS:
                raise ValueError("item keys do not match cache-v5 smoke schema")
...
def freeze_cache_directory(path: str | Path) -> None:
    ...
    if directory.is_symlink() or not directory.is_dir():
        raise ValueError("cache-v5 root must be a real directory")
