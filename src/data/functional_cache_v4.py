"""Crash-safe schema and storage for the Phase 9R eight-item cache-v4 smoke."""
...
CACHE_V4_ITEM_PROTOCOL = "functional-cache-v4-smoke-item-v1"
CACHE_V4_MANIFEST_PROTOCOL = "functional-cache-v4-smoke-manifest-v1"
STATE_DTYPE = np.dtype(np.uint16)
SHA256_PATTERN = re.compile(r"[0-9a-f]{64}")

CACHE_V4_ITEM_KEYS = frozenset(
    {
        "input_ids",
        "attention_mask",
        "answer_start",
...
CACHE_V4_MANIFEST_KEYS = frozenset(
    {
        "protocol",
        "huginn_model_id",
        "huginn_revision",
...
def build_manifest(
    *,
    ...
    manifest = {
        "protocol": CACHE_V4_MANIFEST_PROTOCOL,
...
def validate_manifest(manifest: Mapping[str, Any]) -> None:
    if set(manifest) != CACHE_V4_MANIFEST_KEYS:
        raise ValueError("cache-v4 manifest keys do not match the required schema")
    fixed = {
        "protocol": CACHE_V4_MANIFEST_PROTOCOL,
        "huginn_model_id": HUGINN_MODEL_ID,
        "model_compute_dtype": "bfloat16",
        ...
        "h16_semantics": NORMALIZED_H16_SEMANTICS,
        "full_vocabulary_logits_persisted": False,
        "cache_output": "/workspace/functional_cache_v4_smoke",
    }
    for key, expected in fixed.items():
        if manifest[key] != expected:
            raise ValueError(f"cache-v4 manifest {key} does not match the frozen smoke protocol")
    for key in ("huginn_revision", "tokenizer_revision", "git_commit"):
        if not isinstance(manifest[key], str) or not re.fullmatch(r"[0-9a-f]{40}", manifest[key]):
            raise ValueError(f"cache-v4 manifest {key} must be a 40-character commit")
    for key in ("tokenizer_id", "extraction_location", "source_continuations"):
        if not isinstance(manifest[key], str) or not manifest[key].strip():
            raise ValueError(f"cache-v4 manifest {key} must be nonempty")
    ...
    if not isinstance(hashes, dict) or set(hashes) != {str(i) for i in SMOKE_EXAMPLE_IDS}:
        raise ValueError("cache-v4 manifest must contain source hashes for exactly IDs 0-7")
    if not all(isinstance(value, str) and SHA256_PATTERN.fullmatch(value) for value in hashes.values()):
        raise ValueError("invalid source hash in cache-v4 manifest")
    if not isinstance(manifest["base_seed"], int) or manifest["base_seed"] < 0:
        raise ValueError("cache-v4 base seed must be nonnegative")
...
def validate_item(path: str | Path, manifest: Mapping[str, Any]) -> dict[str, Any]:
    ...
    try:
        with np.load(item_path, allow_pickle=False) as archive:
            if set(archive.files) != CACHE_V4_ITEM_KEYS:
                raise ValueError("item keys do not match cache-v4 smoke schema")
            arrays = {key: archive[key] for key in archive.files}
    except Exception as error:
        raise ValueError(f"invalid cache-v4 item {item_path}: {error}") from error
    ...
    if any(value.dtype != STATE_DTYPE for value in states):
        raise ValueError(f"{item_path}: state slices must be lossless BF16 uint16 bits")
    ...
    strings = {
        ...
        "h16_semantics": NORMALIZED_H16_SEMANTICS,
        "cache_protocol": CACHE_V4_ITEM_PROTOCOL,
...
def write_item_atomic(path: str | Path, arrays: Mapping[str, Any], manifest: Mapping[str, Any]) -> str:
    ...
    with _writer_lock(final.parent):
        ...
        published = _publish_no_replace(
            ...
            redundant_reason="a matching final cache-v4 item appeared before promotion",
        )
        _require_or_make_read_only(final)
...
def write_manifest_atomic(path: str | Path, manifest: Mapping[str, Any]) -> str:
    ...
    with _writer_lock(final.parent):
        ...
        published = _publish_no_replace(
            ...
            invalid_category="invalid_concurrent_final",
            redundant_reason="a matching cache-v4 manifest appeared before promotion",
        )
        _require_or_make_read_only(final)
        return status if published else "validated_existing"
...
def freeze_cache_directory(path: str | Path) -> None:
    ...
    if directory.is_symlink() or not directory.is_dir():
        raise ValueError("cache-v4 root must be a real directory")
    for payload in [directory / "manifest.json", *(directory / f"{i:05d}.npz" for i in SMOKE_EXAMPLE_IDS)]:
        if payload.is_symlink() or not payload.is_file() or stat.S_IMODE(payload.stat().st_mode) != 0o444:
            raise ValueError(f"cache-v4 payload is not immutable: {payload}")
    lock = directory / ".functional-cache-v4.lock"
    if lock.exists():
        if lock.is_symlink() or not lock.is_file():
            raise ValueError("cache-v4 writer lock is not a regular file")
        os.chmod(lock, 0o444)
...
def _validate_expected(path: Path, arrays: Mapping[str, Any], manifest: Mapping[str, Any]) -> None:
    ...
    if set(arrays) != CACHE_V4_ITEM_KEYS:
        raise ValueError("requested arrays do not match cache-v4 schema")
    with np.load(path, allow_pickle=False) as archive:
        for key in CACHE_V4_ITEM_KEYS:
...
def _writer_lock(directory: Path):
    lock_path = directory / ".functional-cache-v4.lock"
