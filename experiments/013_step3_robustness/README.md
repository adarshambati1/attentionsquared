...
Tensor batching was rejected after the full production-size A100 gate changed greedy token trajectories in all 17 conditions. The replacement `concurrent-serial-v3` protocol keeps every continuation at batch size one and executes disjoint condition workers concurrently. Its user-approved proportional gate checks exact 64-token parity for every actual production wave, complete natural-stop parity at every depth, and forced 1,024-token parity/OOM stress for the worst-memory D32 and D64 waves before evaluation. The gate and all v3 production evaluations run on the same `NVIDIA H200`; this user-approved hardware substitution changes execution provenance only, not the 3A–3C design or estimands. This validation change does not alter any 3A, 3B, or 3C prompt, condition, seed, cap, score, or estimand. The failed batched-v2 log and the original five serial records remain historical evidence in their separate output roots; v3 writes only to `/workspace/step3_huginn_robustness_concurrent_v3`.

### MATH-500 context-cap amendment

The frozen 2,048-token schedule and complete prompts make a uniform 1,024-new-token budget impossible for MATH-500 beginning at ID 301. The immutable amendment in `configs/013_step3_math500_context_amendment.json` therefore uses `min(1024, 2048 - prompt_tokens)` only for MATH-500. Prompt truncation and schedule extension remain prohibited. Reaching the effective per-example cap is scored as a cap hit under the same explicit-answer policy. Existing base records are admitted unchanged only when the full 1,024-token budget fit; new records persist the prompt length, requested/effective caps, context-limit flag, amendment hash, and implementation commit.
