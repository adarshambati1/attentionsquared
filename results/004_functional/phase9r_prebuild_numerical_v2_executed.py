...
for i,row in enumerate(rows):
 ...
 pl=F.log_softmax(prefix['logits'],dim=-1);ql=F.log_softmax(full['logits'][:,:len(pids)],dim=-1)
 kls=float((pl.exp()*(pl-ql)).sum());top=int((prefix['logits'].argmax(-1)==full['logits'][:,:len(pids)].argmax(-1)).sum())
...
out={'protocol':'phase9r-independent-prebuild-numerical-v2','status':'pass','cache_built':False,'optimizer_steps':0,'examples':records,'global_diagnostics':{'represented_prompt_tokens':total_tokens,'kl_per_token':total_kl/total_tokens,'top1_agreement':total_top1/total_tokens},'diagnostics_only_no_threshold':True}
Path('/workspace/functional_protocol/phase9r_prebuild_numerical_v2.json').write_text(json.dumps(out,indent=2,sort_keys=True)+'\n')
