...
OUTPUT = Path("/workspace/functional_protocol/phase13_cache_quantization_diagnostic_v3.json")
if OUTPUT.exists():
...
for example_id in range(8):
    ...
    for local in mismatch_indices:
        ...
        live_top1_id = int(live_argmax[local].item())
        runner_up_logits = live_selected[0, local].clone()
        runner_up_logits[live_top1_id] = -torch.inf
        live_top2_id = int(runner_up_logits.argmax().item())
        cached_top1_id = int(cached_argmax[local].item())
        ...
        mismatches.append({
            ...
            "live_top1_token_id": live_top1_id,
            "live_top1_probability": float(live_probs[live_top1_id].item()),
            "live_top2_token_id": live_top2_id,
            "live_top2_probability": float(live_probs[live_top2_id].item()),
            "live_top1_minus_top2_probability_margin": float((live_probs[live_top1_id] - live_probs[live_top2_id]).item()),
            "live_top1_minus_top2_logit_margin": float((live_selected[0, local, live_top1_id] - live_selected[0, local, live_top2_id]).item()),
            "cached_top1_token_id": cached_top1_id,
            "cached_top1_probability": float(cached_probs[cached_top1_id].item()),
...
result = {
    "protocol": "phase13-cache-quantization-diagnostic-v3",
