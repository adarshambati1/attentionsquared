...
CONFIG = Path("configs/004_functional_v5_overfit_v2.json")
OUTPUT_ROOT = Path("/workspace/functional_overfit_v5_native_v2")
PHASE10_ARTIFACT = Path("/workspace/functional_protocol/correctness_gate_cache_v5_native_v1.json")
PHASE10_ATTESTATION = ROOT / "results/004_functional/correctness_gate_cache_v5_native_v1_runtime_attestation.json"
PHASE10_ARTIFACT_SHA256 = "3133d247f30b4a5fc17e785ecab4294ee8d055d11fce1b42c2ec896ceb36a9e4"
PHASE10_ATTESTATION_SHA256 = "e35072a22e546cc2b557d6a68549c27cc7ae67d1cc36d3d0a90c6c4792a803b2"
MODEL_ID = "tomg-group-umd/huginn-0125"
MODEL_REVISION = "bb6621b65e90b6a4b9b29ef88dc83866d450470c"
OVERFIT_PROTOCOL = "functional-eight-example-overfit-cache-v5-native-coda-v2"
PRELAUNCH_PROTOCOL = "functional-eight-example-overfit-cache-v5-native-coda-v2-prelaunch-v1"
...
def validate_phase10_prerequisites() -> dict:
    ...
    if (
        artifact.get("protocol") != "functional-correctness-gate-cache-v5-native-v1"
        or artifact.get("status") != "pass"
        or artifact.get("cache_manifest_sha256")
        != "55a5cc5cc77bf969ef200b49dae7d50d63ef97a88e608978902c6e50f5040583"
        or artifact.get("initialization_sha256")
        != "92968fea30d723864383f68f9e51ef1f8b8adae927e11acf735c3c1cf9b93747"
    ):
        raise ValueError("Phase 10 cache-v5 native artifact is not authoritative")
    if (
        attestation.get("protocol")
        != "functional-correctness-gate-cache-v5-native-v1-runtime-attestation-v1"
        ...
        raise ValueError("Phase 10 cache-v5 native attestation is invalid")
    return attestation
        ...
        examples.append(
            {
                ...
                "h0": uint16_to_bfloat16_tensor(arrays["h0"], device=device)[None].float(),
...
def transient_teacher_logits(huginn, example: dict) -> torch.Tensor:
    with torch.no_grad(), torch.autocast(device_type="cuda", dtype=torch.bfloat16):
        return coda_logits(
            huginn, example, example["h16_teacher"]
        ).detach()
...
def compute_metrics(a2, huginn, examples: list[dict]) -> dict:
    ...
    with torch.no_grad(), torch.autocast(device_type="cuda", dtype=torch.bfloat16):
        for example in examples:
            ...
            student = coda_logits(huginn, example, output[:, 15])
...
def main(config_path: Path, output_root: Path, prelaunch_attestation: Path) -> dict:
    ...
    if sha256_file(CACHE_ROOT / "manifest.json") != config["cache_manifest_sha256"]:
        raise ValueError("cache-v5 manifest checksum mismatch")
