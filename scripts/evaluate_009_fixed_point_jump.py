...
def main():
 ...
 if comparison.exists() or test_log.exists():raise FileExistsError('held-out evaluation output already exists; preserve it and use a new attempt directory')
 ...
 if checkpoint.get('protocol')!='functional-fixed-point-jump-checkpoint-v1' or checkpoint.get('config_sha256')!=config_sha or checkpoint.get('git_commit')!=commit or checkpoint.get('step')!=selection.get('best_step'):raise RuntimeError('checkpoint payload provenance mismatch')
 ...
 for warm in range(c['generation_warmup_examples']):
  prompt=tokenize_prompt(tok,build_chat_prompt(tok,test[warm]['question'],c['system_instruction']))['input_ids'].cuda();schedule=materialize_h0_schedule(huginn,device=prompt.device,example_id=warm,base_seed=c['h0_base_seed']);generate_cached(wrapper,tok,prompt,schedule,depth=0,mode='jump',max_new_tokens=32)
 ...
 with torch.inference_mode():
  for i in range(c['test_ids'][0],c['test_ids'][1]+1):
   ...
   with torch.autocast('cuda',dtype=torch.bfloat16):out,task,fixed,total,count=losses(wrapper,ids,schedule[:,:ids.shape[1]],item.answer_start,c)
