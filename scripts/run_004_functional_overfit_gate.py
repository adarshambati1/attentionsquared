#!/usr/bin/env python3
"""Run the corrected eight-example functional overfit gate and then stop."""
...
from src.training.overfit_gate import (
    OVERFIT_PROTOCOL,
    gate_passed,
    quantitative_overfit_checks,
)


CACHE_ROOT = Path("/workspace/functional_cache_v2")
INITIALIZATION = Path("/workspace/functional_protocol/a2_init_seed_0.pt")
CONFIG = Path("configs/004_functional_v2_overfit.json")
OUTPUT_ROOT = Path("/workspace/functional_overfit_v2")
MODEL_ID = "tomg-group-umd/huginn-0125"
MODEL_REVISION = "bb6621b65e90b6a4b9b29ef88dc83866d450470c"
PRELAUNCH_PROTOCOL = "functional-eight-example-overfit-prelaunch-v1"
...
def coda_logits(huginn, example: dict, state: torch.Tensor) -> torch.Tensor:
    return huginn(
...
@contextmanager
def patched_attention2_recurrence(huginn, a2, example_id: int) -> Iterator[None]:
    original = huginn.iterate_forward
    ...
    def jumped(
        ...
        if input_states is not None:
            raise RuntimeError("overfit generation expects full-prefix fresh states")
        ...
        token_mask = torch.ones(
            input_embeds.shape[:2], dtype=torch.bool, device=input_embeds.device
        )
        with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
            output, _, _ = a2(
                h0.float(), input_embeds.float(), token_mask=token_mask
            )
        return (
            this.transformer.ln_f(output[:, 15].to(h0.dtype)),
            0,
            0,
            h0.detach(),
            block_idx,
    ...
    huginn.iterate_forward = MethodType(jumped, huginn)
    try:
        yield
    finally:
        huginn.iterate_forward = original
...
def generate_same_examples(a2, huginn, tokenizer, examples: list[dict], max_new: int) -> list[dict]:
    stops = {token for token in stop_token_ids(tokenizer) if token is not None and token >= 0}
    generations = []
    a2.eval()
    for example in examples:
        prompt = example["input_ids"][:, : example["answer_start"]]
        generated = []
        with patched_attention2_recurrence(huginn, a2, example["example_id"]):
            for _ in range(max_new):
                current = torch.cat(
                    [prompt, torch.tensor([generated], device=prompt.device, dtype=prompt.dtype)],
                    dim=1,
                )
                attention = torch.ones_like(current, dtype=torch.bool)
