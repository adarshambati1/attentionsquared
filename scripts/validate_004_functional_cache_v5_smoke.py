#!/usr/bin/env python3
"""Independent production validation/replay for the Phase 9R cache-v5 smoke."""
...
CONFIG = Path("configs/004_functional_cache_v5_smoke.json")
CACHE_ROOT = Path("/workspace/functional_cache_v5_smoke")
...
IDS = (0, 1, 2, 3, 4, 5, 6, 7, 2277, 2473, 2452, 2485, 2415, 2264, 2317, 2439)
EXPECTED_BUILDER_COMMIT = "9e9c1fc322f61a23f9f902785de66fba1504cccf"
LOCKED_HASHES = {
...
def _load_locked_inputs() -> tuple[dict, list[dict]]:
    ...
    fixed = {
        "protocol": "functional-cache-v5-smoke-build-v1",
        "model_id": MODEL_ID,
        ...
        "state_dtype": "h0=bfloat16-bits-uint16;x=float32;h16=float32",
        "hidden_size": 5280,
        "base_seed": 3000,
        "seed_index": 0,
    ...
    with (ROOT / config["valid_end_manifest"]).open() as stream:
        for line in stream:
            row = json.loads(line)
            all_rows[row["example_id"]] = row
    rows = [all_rows[example_id] for example_id in IDS]
    for example_id, row in zip(IDS, rows):
        expected_role = "gradient_update" if example_id < 8 else "checkpoint_selection_validation"
        required = {"example_id":example_id,"dataset_id":"openai/gsm8k","dataset_config":"main","dataset_revision":DATASET_REVISION,"dataset_split":"train","phase3_role":expected_role,"source_sha256":RAW_HASHES[str(example_id)]}
...
def validate_file_set(root: Path = CACHE_ROOT) -> list[Path]:
    ...
    allowed = {root / ".functional-cache-v5.lock"}
...
def validate_static(root: Path = CACHE_ROOT):
    ...
    manifest_fixed = {
        "protocol": "functional-cache-v5-smoke-manifest-v1",
        "huginn_model_id": MODEL_ID,
        "huginn_revision": MODEL_REVISION,
        "tokenizer_id": MODEL_ID,
        "tokenizer_revision": MODEL_REVISION,
        "model_compute_dtype": "bfloat16-autocast; native hidden outputs float32",
        "state_dtype": "h0=bfloat16-bits-uint16;x=float32;h16=float32",
        "hidden_size": 5280,
        "extraction_location": "explicit fixed-schedule BF16 input_states -> frozen canonical Huginn D16; h0 persisted as raw BF16 uint16 bits; x post-prelude recurrent input and normalized pre-coda h16 persisted as native FP32",
        "example_ids": list(IDS),
        "sequence_count": 16,
    ...
    values = []
    for example_id, row, item in zip(IDS, rows, items):
        ...
        string_expected = {
            ...
            "cache_protocol": "functional-cache-v5-smoke-item-v1",
        ...
        expected_dtypes = {"h0": np.uint16, "x": np.float32, "h16": np.float32}
        for key, expected_dtype in expected_dtypes.items():
            if value[key].shape != (len(raw), 5280) or value[key].dtype != expected_dtype or not np.isfinite(value[key]).all():
                raise ValueError(f"cached native state schema failed {key} ID={example_id}")
        values.append(value)
...
def replay(config, rows, manifest, values):
    ...
    try:
        for example_id, row, cached in zip(IDS, rows, values):
            ...
            for key in ("x", "h16"):
                observed = teacher[key].detach().cpu().contiguous().numpy()[0]
                if observed.dtype != np.float32 or observed.tobytes() != cached[key].tobytes():
                    raise ValueError(
                        f"cached {key} FP32 bits are not byte-identical ID={example_id}"
                    )
            cached_h16 = torch.from_numpy(cached["h16"].copy())[None].cuda()
            if not torch.equal(cached_h16, teacher["h16"]):
                raise ValueError(f"loaded cached h16 differs from live FP32 ID={example_id}")
...
def main():
    ...
    print(json.dumps({"protocol":"functional-cache-v5-smoke-independent-validation-v3","status":"pass","cache_builder_git_commit":manifest["git_commit"],"h0_exactness_hard_gate":True,"full_schedule_persisted":False,"logits_persisted":False,**result}, indent=2, sort_keys=True))
