...
def main(config_path, data_path, output_dir, depth_scale=None, film=False, objective='mse_endpoint', eta=1.0):
 ...
 for step in range(1,c['train_steps']+1):
  ...
  if objective=='normalized_trajectory':
   p=z.float(); y=target[i].float(); token_valid=mask[i][None,:].expand(d,t); rel=((p-y).norm(dim=-1)/(y.norm(dim=-1)+1e-8))[token_valid].mean(); cos=(1-torch.nn.functional.cosine_similarity(p[token_valid],y[token_valid],dim=-1)).mean(); loss=rel+eta*cos
  else:
