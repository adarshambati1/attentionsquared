#!/usr/bin/env python3
"""Build only the immutable eight-item Phase 9R prefix-stable cache-v5 smoke."""
...
from src.data.functional_cache_v5 import (CACHE_V5_ITEM_PROTOCOL, SMOKE_EXAMPLE_IDS,
...
CONFIG=Path("configs/004_functional_cache_v5_smoke.json"); OUTPUT=Path("/workspace/functional_cache_v5_smoke")
...
def locked_rows(c):
 ...
 rows=[all_rows[i] for i in SMOKE_EXAMPLE_IDS]
 for row in rows:
...
def preflight_estimate(c,rows):
 ...
 return {"item_count":len(SMOKE_EXAMPLE_IDS),"total_tokens":tokens,"estimated_uncompressed_item_bytes":payload,"largest_transient_schedule_bytes":2048*c["hidden_size"]*2,"minimum_free_bytes_with_2x_margin":payload*2}
...
def main(config_path:Path,preflight_only=False):
 ...
 print(json.dumps({"protocol":"functional-cache-v5-smoke-preflight-v2",**estimate,"storage":storage},sort_keys=True))
 ...
 manifest=build_manifest(model_revision=c["model_revision"],tokenizer_id=c["model_id"],tokenizer_revision=c["model_revision"],tokenizer_template=tok.chat_template,git_commit=commit,extraction_location=EXTRACTION_LOCATION_V5,source_continuations=c["source_continuations"],source_sha256_by_id={i:c["raw_source_sha256"][str(i)] for i in SMOKE_EXAMPLE_IDS},base_seed=3000,seed_index=0,cache_output=str(OUTPUT),split_manifest_sha256=c["split_manifest_sha256"],source_cache_v2_manifest_sha256=c["source_cache_v2_manifest_sha256"],source_cache_v2_freeze_sha256=c["source_cache_v2_freeze_sha256"],estimated_uncompressed_item_bytes=estimate["estimated_uncompressed_item_bytes"])
 ...
 if {p.name for p in attempt.iterdir()}!={"manifest.json",".functional-cache-v5.lock",*(f"{i:05d}.npz" for i in SMOKE_EXAMPLE_IDS)}: raise RuntimeError("non-exact staging file set")
