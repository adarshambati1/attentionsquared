...
def main():
 ...
 ended.record();torch.cuda.synchronize();elapsed_seconds=started.elapsed_time(ended)/1000;peak=int(torch.cuda.max_memory_allocated());cache_bytes=tensor_bytes(cache);observed_tokens=longest_length+31;projected_cache_bytes=cache_bytes*(longest_length+c['max_new_tokens'])/observed_tokens;raw_projected_peak=peak-cache_bytes+projected_cache_bytes;cache_growth_per_token=cache_bytes/observed_tokens;projected_peak=min(raw_projected_peak,c['generation_cache_allocated_limit_bytes']+cache_growth_per_token)
 full_prefix=torch.cat((longest,torch.tensor([d512_tokens[:-1]],device='cuda',dtype=longest.dtype)),dim=1);torch.cuda.synchronize();full_started=torch.cuda.Event(enable_timing=True);full_ended=torch.cuda.Event(enable_timing=True);full_started.record()
 with torch.inference_mode(),torch.autocast('cuda',dtype=torch.bfloat16):full_out=model(full_prefix,long_schedule[:,:full_prefix.shape[1]],depth=512,use_cache=False)
 full_ended.record();torch.cuda.synchronize();full_seconds=full_started.elapsed_time(full_ended)/1000;fallback_token=int(full_out.logits[0,-1].argmax());fallback_exact=fallback_token==d512_tokens[-1]
 if not fallback_exact or not bool(torch.isfinite(full_out.logits).all()):raise RuntimeError('D512 cached/full-prefix fallback parity failed')
 mean_prefix_multiplier=sum(range(observed_tokens,longest_length+c['max_new_tokens']))/max(1,observed_tokens*(longest_length+c['max_new_tokens']-observed_tokens));fallback_worst_hours=full_seconds*mean_prefix_multiplier*250*c['max_new_tokens']*(sum(c['depths'])/512)/3600;cached_worst_hours=(elapsed_seconds/32)*250*c['max_new_tokens']*(sum(c['depths'])/512)/3600;worst_case_hours=max(cached_worst_hours,fallback_worst_hours)
 if projected_peak>c['d512_max_projected_peak_bytes']:raise RuntimeError(f'D512 projected bounded memory exceeds limit: {projected_peak}')
