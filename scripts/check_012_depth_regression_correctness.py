...
def main():
 ...
 c=json.loads(CONFIG.read_text());root=Path(c['output_root']);root.mkdir(exist_ok=True);output=root/'correctness_gate.json'
 ...
 with torch.inference_mode(),torch.autocast('cuda',dtype=torch.bfloat16):
  for depth in c['depths']:
   ...
   for step in range(8):
    input_h0=schedule[:,:current.shape[1]] if position is None else schedule[:,position:position+1];cp=None if position is None else torch.tensor([position],device='cuda');native=huginn(input_ids=current,input_states=input_h0,num_steps=depth,past_key_values=native_cache,use_cache=True,cache_position=cp,return_dict=True,output_details={'return_logits':True,'return_latents':True,'return_head':False,'return_stats':False});wrapped=model(current,input_h0,depth=depth,past_key_values=wrapped_cache,use_cache=True,cache_position=cp);native_cache=native.past_key_values;wrapped_cache=wrapped.past_key_values;delta=float((native.logits[:,-1].float()-wrapped.logits[:,-1]).abs().max());native_token=int(native.logits[0,-1].argmax());wrapped_token=int(wrapped.logits[0,-1].argmax());steps.append({'step':step,'generated_token_exact':native_token==wrapped_token,'native_stop':native_token in stops,'wrapped_stop':wrapped_token in stops,'max_logit_abs_error':delta})
    if native_token!=wrapped_token or (native_token in stops)!=(wrapped_token in stops) or delta>.25:raise RuntimeError({'depth':depth,'steps':steps})
 ...
 if projected_peak>75*(1024**3):raise RuntimeError(f'D512 projected memory exceeds 75 GiB: {projected_peak}')
 worst_case_hours=(elapsed_seconds/32)*250*c['max_new_tokens']*(sum(c['depths'])/512)/3600;unchanged=before==sha(huginn);result={'protocol':c['protocol']+'-correctness-gate','status':'pass','native_parity':native_checks,'cache_vs_full_prefix':cache_checks,'d512_preflight':{'longest_prompt_example_id':longest_id,'longest_prompt_tokens':longest_length,'generated_tokens':d512_tokens,'finite':bool(torch.isfinite(out.logits).all()),'elapsed_seconds_32_steps':elapsed_seconds,'seconds_per_step':elapsed_seconds/32,'peak_memory_bytes':peak,'observed_cache_bytes':cache_bytes,'projected_peak_bytes_at_prompt_plus_cap':projected_peak,'projected_worst_case_all_depths_hours':worst_case_hours,'memory_headroom_threshold_bytes':75*(1024**3)},'huginn_weights_unchanged':unchanged,'new_parameters':0,'config_sha256':hashlib.sha256(CONFIG.read_bytes()).hexdigest(),'git_commit':subprocess.run(['git','rev-parse','HEAD'],cwd=ROOT,text=True,capture_output=True,check=True).stdout.strip(),'full_vocabulary_logits_persisted':False}
