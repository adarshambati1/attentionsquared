...
def validate(r,*,dataset_name,model,depth,seed_index,example_id,c,commit,tok,item):
 required=('generated_text','generated_token_ids','correct','predicted_answer','gold_answer','generated_tokens','hit_max_new_tokens','ended_naturally','generation_latency_seconds','peak_memory_bytes','repetition_onset','degeneration','used_full_prefix_fallback','full_prefix_fallback_onset')
 if any(k not in r for k in required) or any((r.get(k)!=v) for k,v in {'protocol':c['protocol']+'-record','dataset':dataset_name,'model':model,'depth':depth,'seed_index':seed_index,'example_id':example_id,'git_commit':commit,'config_sha256':sha(ROOT/'configs/013_step3_huginn_robustness.json'),'hardware':c['hardware'],'checkpoint_sha256_before':c['_checkpoint_hashes_observed'],'gate_sha256':c['_gate_sha256'],'concurrency_gate_sha256':c['_concurrency_gate_sha256'],'checkpoint_set_sha256':hashlib.sha256(json.dumps(c['_checkpoint_hashes_observed'],sort_keys=True).encode()).hexdigest()}.items()):raise RuntimeError(f'record provenance/schema mismatch {dataset_name}/{model}/D{depth}/S{seed_index}/ID{example_id}')
...
def main():
 ...
 validate_correctness_gate_contract(gate,config=c,commit=commit,config_sha256=sha(cp),hardware=c['hardware'])
 validate_gate_contract(concurrency_gate,config=c,commit=commit,config_sha256=sha(cp),correctness_gate_sha256=gate_sha,hardware=c['hardware'])
 ...
 for model,depth in conditions:
  ...
  for seed_index in c['h0_seed_indices']:
   ...
   for path in sorted(directory.glob('*.json')):
    ...
    r=json.loads(path.read_text());validate(r,dataset_name=a.dataset,model=model,depth=depth,seed_index=seed_index,example_id=expected,c=c,commit=commit,tok=tok,item=dataset[expected]);existing.append(r)
   records.extend(existing)
   for example_id in ids[len(existing):]:
    question,gold=question_and_gold(a.dataset,dataset[example_id]);prompt=tokenize_prompt(tok,build_chat_prompt(tok,question,c['system_instruction']))['input_ids'].cuda()
 ...
 result={'protocol':c['protocol']+f'-{a.dataset}-evaluation','status':'complete','dataset':a.dataset,'conditions':summaries,'records':len(records),'git_commit':commit,'config_sha256':sha(cp),'checkpoint_sha256_before':checkpoint_hashes_before,'checkpoint_sha256_after':checkpoint_hashes_after,'checkpoint_set_sha256':checkpoint_set_observed,'gate_sha256':gate_sha,'concurrency_gate_sha256':sha(concurrency_gate_path),'hardware':torch.cuda.get_device_name(),'generation_latency_semantics':'worker wall-clock under concurrent condition execution; operational only and not comparable to serial per-example latency','full_vocabulary_logits_persisted':False,'do_not_start_step_4':True};publish(root/f'{a.dataset}_summary.json',result,True);print(json.dumps(result,indent=2,sort_keys=True))
