...
def main():
 ...
 for model,depth in conditions:
  wrapper=wrappers[model];mode=model_mode(model);name=condition_name(model,depth)
 ...
 result={'protocol':c['protocol']+f'-{a.dataset}-evaluation','status':'complete','dataset':a.dataset,'conditions':summaries,'records':len(records),'git_commit':commit,'config_sha256':sha(cp),'checkpoint_sha256_before':checkpoint_hashes_before,'checkpoint_sha256_after':checkpoint_hashes_after,'checkpoint_set_sha256':checkpoint_set_observed,'gate_sha256':gate_sha,'hardware':torch.cuda.get_device_name(),'full_vocabulary_logits_persisted':False,'do_not_start_step_4':True};publish(root/f'{a.dataset}_summary.json',result,True);print(json.dumps(result,indent=2,sort_keys=True))
