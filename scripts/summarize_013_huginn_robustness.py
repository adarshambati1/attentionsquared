...
def metrics(rs,seeds):
 by_seed={str(s):sum(r['correct'] for r in rs if r['seed_index']==s)/sum(r['seed_index']==s for r in rs) for s in seeds};batches={r['batch_id']:(r['batch_elapsed_seconds'],r['batch_generated_tokens_total']) for r in rs};elapsed=sum(v[0] for v in batches.values());tokens=sum(v[1] for v in batches.values());return {'by_seed_accuracy':by_seed,'correct':sum(r['correct'] for r in rs),'total':len(rs),'pooled_accuracy':sum(r['correct'] for r in rs)/len(rs),'mean_seed_accuracy':sum(by_seed.values())/len(by_seed),'cap_hit_rate':sum(r['hit_max_new_tokens'] for r in rs)/len(rs),'degeneration_rate':sum(r['degeneration'] for r in rs)/len(rs),'mean_output_length':sum(r['generated_tokens'] for r in rs)/len(rs),'batched_generated_tokens_per_second':tokens/elapsed,'total_batch_wall_seconds':elapsed,'batch_count':len(batches),'latency_estimand':'fixed-forward latency is comparable; generation throughput is amortized over deterministic exact-length batches'}
def main():
 ...
 commit=subprocess.run(['git','rev-parse','HEAD'],cwd=ROOT,text=True,capture_output=True,check=True).stdout.strip();config_sha=hashlib.sha256(CONFIG.read_bytes()).hexdigest();gate_sha=hashlib.sha256((root/'correctness_gate.json').read_bytes()).hexdigest();checkpoint_set=hashlib.sha256(json.dumps(c['checkpoint_sha256'],sort_keys=True).encode()).hexdigest();summaries={}
 ...
 for dataset in ('gsm8k','svamp','math500'):
  ...
  if s.get('status')!='complete' or s.get('git_commit')!=commit or s.get('config_sha256')!=config_sha or s.get('checkpoint_sha256_before')!=c['checkpoint_sha256'] or s.get('checkpoint_sha256_after')!=c['checkpoint_sha256'] or s.get('checkpoint_set_sha256')!=checkpoint_set or s.get('gate_sha256')!=gate_sha or s.get('hardware')!=c['hardware'] or sorted(s.get('conditions',{}))!=sorted(expected_conditions[dataset]) or s.get('records')!=expected_per*len(expected_conditions[dataset]):raise RuntimeError(f'{dataset} summary provenance/coverage mismatch')
  summaries[dataset]=s;batch_manifest_path=root/f'{dataset}_batch_manifest.json';batch_manifest=json.loads(batch_manifest_path.read_text());batch_manifest_sha=hashlib.sha256(batch_manifest_path.read_bytes()).hexdigest()
  if s.get('batch_manifest_sha256')!=batch_manifest_sha:raise RuntimeError(f'{dataset} summary/batch-manifest binding mismatch')
  if batch_manifest.get('protocol')!=c['protocol']+f'-{dataset}-batch-manifest' or batch_manifest.get('git_commit')!=commit or batch_manifest.get('config_sha256')!=config_sha or sorted(batch_manifest.get('conditions',{}))!=sorted(expected_conditions[dataset]):raise RuntimeError(f'{dataset} batch manifest provenance mismatch')
  batch_lookup={}
  for condition,batches in batch_manifest['conditions'].items():
   for batch in batches:
    for index,member in enumerate(batch['members']):batch_lookup[(condition,member['example_id'],member['seed_index'])]=(batch,index)
  for name in expected_conditions[dataset]:
   ...
   for path,r in zip(paths,rs):
    ...
    batch,index=batch_lookup[(name,r['example_id'],r['seed_index'])];measurement_path=root/'batch_measurements'/dataset/name/f"{batch['batch_id']}.json";measurement=json.loads(measurement_path.read_text());measurement_sha=hashlib.sha256(measurement_path.read_bytes()).hexdigest();fixed={'protocol':c['protocol']+'-record','dataset':dataset,'model':model,'depth':depth,'git_commit':commit,'config_sha256':config_sha,'hardware':c['hardware'],'checkpoint_sha256_before':c['checkpoint_sha256'],'gate_sha256':gate_sha,'checkpoint_set_sha256':checkpoint_set,'h0_seed':per_example_seed(c['h0_base_seed'],r['example_id'],step=0,seed_index=r['seed_index']),'batch_id':batch['batch_id'],'batch_member_index':index,'generation_batch_size':len(batch['members']),'batch_manifest_sha256':batch_manifest_sha,'batch_measurement_sha256':measurement_sha,'batch_elapsed_seconds':measurement['batch_elapsed_seconds'],'batch_generated_tokens_total':measurement['batch_generated_tokens_total'],'peak_memory_bytes':measurement['peak_memory_bytes']}
    if path!=expected_path or any(r.get(k)!=v for k,v in fixed.items()) or r['generated_text']!=tokenizer.decode(r['generated_token_ids'],skip_special_tokens=False) or any(r.get(k)!=v for k,v in rescored.items()) or r['generated_tokens']!=len(r['generated_token_ids']) or r['hit_max_new_tokens']!=status.hit_max_new_tokens or r['ended_naturally']!=status.ended_naturally or r['repetition_onset']!=repetition or r['degeneration']!=(repetition is not None) or r.get('generation_execution')!='exact-length-batched-greedy' or not isinstance(r.get('generation_batch_size'),int) or not 1<=r['generation_batch_size']<=c['generation_batch_size_by_depth'][str(depth)] or not isinstance(r['used_full_prefix_fallback'],bool) or (r['used_full_prefix_fallback']!=(r['full_prefix_fallback_onset'] is not None)) or (r['full_prefix_fallback_onset'] is not None and (not isinstance(r['full_prefix_fallback_onset'],int) or not 1<=r['full_prefix_fallback_onset']<=r['generated_tokens'])) or not isinstance(r['batch_wall_seconds_to_row_stop'],(int,float)) or not math.isfinite(r['batch_wall_seconds_to_row_stop']) or not 0<=r['batch_wall_seconds_to_row_stop']<=r['batch_elapsed_seconds'] or not isinstance(r['peak_memory_bytes'],int) or r['peak_memory_bytes']<0:raise RuntimeError(f'{dataset}/{name} strict record validation failed: {path}')
   record_lookup={(r['example_id'],r['seed_index']):r for r in rs}
   for batch in batch_manifest['conditions'][name]:
    measurement_path=root/'batch_measurements'/dataset/name/f"{batch['batch_id']}.json";measurement=json.loads(measurement_path.read_text());ordered=[record_lookup[(m['example_id'],m['seed_index'])]['generated_token_ids'] for m in batch['members']];sequence_sha=hashlib.sha256(json.dumps(ordered,separators=(',',':')).encode()).hexdigest()
    member_results=measurement.get('member_results',[]);member_exact=len(member_results)==len(batch['members']) and all(member_results[index].get('token_ids')==record_lookup[(member['example_id'],member['seed_index'])]['generated_token_ids'] and member_results[index].get('latency_seconds')==record_lookup[(member['example_id'],member['seed_index'])]['batch_wall_seconds_to_row_stop'] and member_results[index].get('used_full_prefix_fallback')==record_lookup[(member['example_id'],member['seed_index'])]['used_full_prefix_fallback'] and member_results[index].get('full_prefix_fallback_onset')==record_lookup[(member['example_id'],member['seed_index'])]['full_prefix_fallback_onset'] for index,member in enumerate(batch['members']))
    if measurement.get('protocol')!=c['protocol']+'-batch-measurement' or measurement.get('batch_id')!=batch['batch_id'] or measurement.get('ordered_members')!=batch['members'] or measurement.get('batch_manifest_sha256')!=batch_manifest_sha or measurement.get('token_sequences_sha256')!=sequence_sha or measurement.get('git_commit')!=commit or measurement.get('config_sha256')!=config_sha or not member_exact:raise RuntimeError(f'{dataset}/{name} batch measurement reconstruction failed: {batch["batch_id"]}')
 ...
 result={'protocol':c['protocol']+'-summary-3abc','status':'complete','inference_estimand':'Accuracy averaged over the three frozen deterministic h0 seeds. Confidence intervals resample example IDs as clusters and average the three fixed seeds; they quantify held-out-example uncertainty conditional on these seeds and do not claim generalization to arbitrary initialization seeds.','table1_gsm8k_paired_seeds':table1,'table1_paired_comparisons':paired1,'table2_cross_dataset':table2,'table2_paired_comparisons':paired2,'table3_depth_generalization':table3,'table3_paired_comparisons':paired3,'fixed_forward_latency':latency,'git_commit':commit,'config_sha256':config_sha,'checkpoint_sha256':c['checkpoint_sha256'],'gate_sha256':gate_sha,'hardware':c['hardware'],'do_not_start_step_4':True};publish(root/'summary_3abc.json',result);print(json.dumps(result,indent=2,sort_keys=True))
