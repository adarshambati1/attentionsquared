#!/usr/bin/env python3
"""Train one frozen D8 history attribution control with the original budget."""
    ...
    with torch.inference_mode():
        for example_id in ids:
            ...
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                output = wrapper(input_ids, schedule[:, : input_ids.shape[1]], depth=8, mode=config["history_mode"])
...
def checkpoint_payload(wrapper, optimizer, step, validation, config_path, config):
    return {
        "protocol": f"{config['variant']}-d8-checkpoint-v1",
...
def main() -> None:
    ...
    wrapper = wrapper_class(huginn, config["projection_size"], config["attention_heads"]).cuda().train()
    ...
    for step in range(1, config["optimizer_steps"] + 1):
        ...
        for example_id, item in batch:
            ...
            schedule = materialize_h0_schedule(huginn, device=input_ids.device, example_id=example_id, base_seed=config["h0_base_seed"])
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                output_value = wrapper(input_ids, schedule[:, : input_ids.shape[1]], depth=8, mode=config["history_mode"])
        ...
        if step == config["startup_gate_step"]:
            ...
            generation = generate_cached(wrapper, tokenizer, prompt_ids, schedule, depth=8, mode=config["history_mode"], max_new_tokens=32)
            ...
            atomic_torch_save(test_path, checkpoint_payload(wrapper, optimizer, step, startup_loss, config_path, config))
            with curve.open("a") as stream:
        ...
        if step % config["validation_every_steps"] == 0:
            ...
            checkpoint = checkpoint_payload(wrapper, optimizer, step, value, config_path, config)
    ...
    summary = {
        "protocol": f"{config['variant']}-d8-training-v1",
        "variant": config["variant"],
        "status": "complete",
