...
def generate(wrapper,tok,prompt,schedule,*,example_id,perturb_depth,sigma,seed,c):
 ...
 with torch.inference_mode(),torch.autocast('cuda',dtype=torch.bfloat16):
  for _ in range(c['max_new_tokens']):
   h0=schedule[:,:current.shape[1]] if position is None else schedule[:,position:position+1];cp=None if position is None else torch.tensor([position],device='cuda');out=wrapper(current,h0,depth=c['functional_generation_depth'],perturb_depth=perturb_depth,sigma=sigma,example_id=example_id,perturbation_seed_index=seed,perturbation_base_seed=c['perturbation_base_seed'],past_key_values=cache,use_cache=True,cache_position=cp);cache=out.past_key_values;token=int(out.logits[0,-1].argmax());tokens.append(token)
...
def main():
 ...
 for sigma,seed,i in conditions[len(records):]:
  prompt=tokenize_prompt(tok,build_chat_prompt(tok,test[i]['question'],c['system_instruction']))['input_ids'].cuda();schedule=materialize_h0_schedule(huginn,device=prompt.device,example_id=i,base_seed=c['h0_base_seed']);g=generate(wrapper,tok,prompt,schedule,example_id=i,perturb_depth=8,sigma=sigma,seed=seed,c=c);score=score_generation(g['text'],test[i]['answer'],hit_max_new_tokens=g['hit_cap']);r={'sigma':sigma,'perturbation_seed_index':seed,'example_id':i,'config_sha256':config_sha,'noise_seed_derivation':c['noise_seed_derivation'],'correct':score.correct,'predicted_answer':score.predicted_answer,'gold_answer':score.gold_answer,'generated_text':g['text'],'generated_token_ids':g['token_ids'],'generated_tokens':len(g['token_ids']),'generation_latency_seconds':g['latency_seconds'],'peak_memory_bytes':g['peak_memory_bytes'],'hit_max_new_tokens':g['hit_cap'],'repetition_onset':find_repetition_onset(g['token_ids'],chunk_size=c['degeneration_chunk_size'],repetitions=c['degeneration_repetitions']),'clean_d16_correct':clean_records[i]['correct']};records.append(r)
