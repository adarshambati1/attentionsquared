...
def promote_plot(tmp,path):
 if path.exists():
  if path.read_bytes()!=tmp.read_bytes():raise RuntimeError(f'existing plot differs: {path}')
...
def validate_record(r,*,depth,example_id,config_sha,commit,c,tok,test):
 required=('generated_text','generated_token_ids','extracted_answer','gold_answer','correct','generated_tokens','hit_max_new_tokens','degeneration','generation_latency_seconds','fixed_point_residual','peak_memory_bytes','used_full_prefix_fallback','full_prefix_fallback_onset')
 ...
 if not all(torch.isfinite(torch.tensor(float(r[key]))).item() for key in ('generation_latency_seconds','fixed_point_residual')) or r['generation_latency_seconds']<0 or r['fixed_point_residual']<0 or not isinstance(r['peak_memory_bytes'],int) or r['peak_memory_bytes']<0 or not isinstance(r['generated_tokens'],int) or r['generated_tokens']<1:raise RuntimeError(f'non-finite record D={depth} ID={example_id}')
 if not isinstance(r['used_full_prefix_fallback'],bool) or (r['used_full_prefix_fallback']!=(isinstance(r['full_prefix_fallback_onset'],int) and 1<=r['full_prefix_fallback_onset']<=r['generated_tokens'])):raise RuntimeError(f'invalid cache fallback metadata D={depth} ID={example_id}')
...
def summarize(records):return {'correct':sum(r['correct'] for r in records),'total':len(records),'accuracy':sum(r['correct'] for r in records)/len(records),'fixed_point_residual_mean':sum(r['fixed_point_residual'] for r in records)/len(records),'fixed_point_residual_median':statistics.median(r['fixed_point_residual'] for r in records),'cap_hit_rate':sum(r['hit_max_new_tokens'] for r in records)/len(records),'degeneration_rate':sum(r['repetition_onset'] is not None for r in records)/len(records),'mean_generated_tokens':sum(r['generated_tokens'] for r in records)/len(records),'mean_generation_latency_seconds':sum(r['generation_latency_seconds'] for r in records)/len(records),'median_generation_latency_seconds':statistics.median(r['generation_latency_seconds'] for r in records),'peak_memory_bytes':max(r['peak_memory_bytes'] for r in records),'full_prefix_fallbacks':sum(r['used_full_prefix_fallback'] for r in records)}
def main():
 ...
 cp=ROOT/a.config;c=json.loads(cp.read_text());config_sha=hashlib.sha256(cp.read_bytes()).hexdigest();commit=subprocess.run(['git','rev-parse','HEAD'],cwd=ROOT,text=True,capture_output=True,check=True).stdout.strip();dirty=subprocess.run(['git','status','--porcelain','--untracked-files=no'],cwd=ROOT,text=True,capture_output=True,check=True).stdout
 if dirty:raise RuntimeError(f'tracked worktree is dirty: {dirty}')
 root=Path(c['output_root']);root.mkdir(exist_ok=True);lock=(root/'run.lock').open('a');fcntl.flock(lock,fcntl.LOCK_EX|fcntl.LOCK_NB);gate_path=root/'correctness_gate.json';gate=json.loads(gate_path.read_text());native=gate.get('native_parity',[]);cache_gate=gate.get('cache_vs_full_prefix',[]);preflight=gate.get('d512_preflight',{})
 gate_valid=gate.get('status')=='pass' and gate.get('config_sha256')==config_sha and gate.get('git_commit')==commit and [r.get('depth') for r in native]==c['depths'] and all(r.get('logits_bitwise_exact') and r.get('latents_bitwise_exact') and r.get('fixed_point_residual_finite') for r in native) and [r.get('depth') for r in cache_gate]==c['depths'] and all(r.get('steps') and all(s.get('generated_token_exact') and s.get('native_stop')==s.get('wrapped_stop') and s.get('all_outputs_finite') and isinstance(s.get('max_logit_abs_error'),(int,float)) and torch.isfinite(torch.tensor(float(s['max_logit_abs_error']))).item() for s in r['steps']) for r in cache_gate) and preflight.get('finite') and preflight.get('full_prefix_fallback_token_exact') and preflight.get('projected_peak_bytes_at_prompt_plus_cap',float('inf'))<=c['d512_max_projected_peak_bytes'] and preflight.get('projected_worst_case_all_depths_hours',float('inf'))<=c['d512_max_projected_worst_case_hours'] and gate.get('huginn_weights_unchanged')
 if not gate_valid:raise RuntimeError('correctness gate provenance/content mismatch')
 ...
 for depth in c['depths']:
  ...
  for example_id in range(c['test_ids'][0]+len(records),c['test_ids'][1]+1):
   ...
   schedule=materialize_h0_schedule(huginn,device=prompt.device,example_id=example_id,base_seed=c['h0_base_seed']);residual=measure_prompt_residual(model,prompt,schedule,depth);g=generate_cached(model,tok,prompt,schedule,depth=depth,mode='plain',max_new_tokens=c['max_new_tokens'],max_cache_allocated_bytes=c['generation_cache_allocated_limit_bytes']);score=score_generation(g.text,test[example_id]['answer'],hit_max_new_tokens=g.hit_max_new_tokens);r={'protocol':c['protocol']+'-record','depth':depth,'example_id':example_id,'config_sha256':config_sha,'git_commit':commit,'model_revision':c['model_revision'],'dataset_revision':c['dataset_revision'],'h0_base_seed':c['h0_base_seed'],'h0_seed_index':c['h0_seed_index'],'generated_text':g.text,'generated_token_ids':g.token_ids,'extracted_answer':score.predicted_answer,'gold_answer':score.gold_answer,'correct':score.correct,'generated_tokens':g.generated_tokens,'hit_max_new_tokens':g.hit_max_new_tokens,'repetition_onset':find_repetition_onset(g.token_ids,chunk_size=c['degeneration_chunk_size'],repetitions=c['degeneration_repetitions']),'degeneration':False,'generation_latency_seconds':g.latency_seconds,'peak_memory_bytes':g.peak_memory_bytes,'used_full_prefix_fallback':g.used_full_prefix_fallback,'full_prefix_fallback_onset':g.full_prefix_fallback_onset,'fixed_point_residual':residual};r['degeneration']=r['repetition_onset'] is not None;validate_record(r,depth=depth,example_id=example_id,config_sha=config_sha,commit=commit,c=c,tok=tok,test=test);write_exclusive(directory/f'{example_id:03d}.json',r);records.append(r);print(json.dumps({k:v for k,v in r.items() if k not in ('generated_text','generated_token_ids')}),flush=True)
  ...
  if depth==16:
   ...
   write_exclusive(replay_path,replay,pretty=True)
  ...
  write_exclusive(summary_path,depth_summary,pretty=True)
 transitions=[]
 ...
 fields=['depth','example_id','extracted_answer','gold_answer','correct','generated_tokens','hit_max_new_tokens','degeneration','repetition_onset','generation_latency_seconds','peak_memory_bytes','fixed_point_residual','used_full_prefix_fallback','full_prefix_fallback_onset','generated_text'];csv_buffer=io.StringIO(newline='');w=csv.DictWriter(csv_buffer,fieldnames=fields);w.writeheader();w.writerows({k:r[k] for k in fields} for r in raw);publish_bytes(root/'raw_results.csv',csv_buffer.getvalue().encode())
 write_exclusive(root/'transitions.json',transitions,pretty=True)
 summary={'protocol':c['protocol'],'step':c['step'],'status':'complete','table':table,'transitions':transitions,'config_sha256':config_sha,'git_commit':commit,'correctness_gate_sha256':gate_sha,'raw_records':len(raw),'do_not_start_step_3':True,'full_vocabulary_logits_persisted':False}
