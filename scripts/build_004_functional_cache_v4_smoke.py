#!/usr/bin/env python3
"""Build only the immutable eight-item Phase 9R prefix-stable cache-v4 smoke."""
...
from src.data.functional_cache_v4 import (CACHE_V4_ITEM_PROTOCOL, SMOKE_EXAMPLE_IDS,
...
from src.data.functional_extraction_v4 import EXTRACTION_LOCATION_V4, NORMALIZED_H16_SEMANTICS, capture_scheduled_huginn_states
...
CONFIG=Path("configs/004_functional_cache_v4_smoke.json"); OUTPUT=Path("/workspace/functional_cache_v4_smoke")
...
def main(config_path:Path,preflight_only=False):
 ...
 print(json.dumps({"protocol":"functional-cache-v4-smoke-preflight-v2",**estimate,"storage":storage},sort_keys=True))
 ...
 manifest=build_manifest(model_revision=c["model_revision"],tokenizer_id=c["model_id"],tokenizer_revision=c["model_revision"],tokenizer_template=tok.chat_template,git_commit=commit,extraction_location=EXTRACTION_LOCATION_V4,source_continuations=c["source_continuations"],source_sha256_by_id={i:c["raw_source_sha256"][str(i)] for i in SMOKE_EXAMPLE_IDS},base_seed=3000,seed_index=0,cache_output=str(OUTPUT),split_manifest_sha256=c["split_manifest_sha256"],source_cache_v2_manifest_sha256=c["source_cache_v2_manifest_sha256"],source_cache_v2_freeze_sha256=c["source_cache_v2_freeze_sha256"],estimated_uncompressed_item_bytes=estimate["estimated_uncompressed_item_bytes"])
 ...
 if {p.name for p in attempt.iterdir()}!={"manifest.json",".functional-cache-v4.lock",*(f"{i:05d}.npz" for i in SMOKE_EXAMPLE_IDS)}: raise RuntimeError("non-exact staging file set")
 os.chmod(attempt/".functional-cache-v4.lock",0o444)
