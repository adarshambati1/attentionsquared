...
from src.evaluation.correctness import SEED_PROTOCOL, build_chat_prompt, ensure_new_output_path, seed_for_example, tokenize_prompt, write_json_exclusive
...
def main(config_path: Path, output: Path) -> None:
    ...
    records=[]; validated=[]; derived_seeds=[]; state_errors=[]
    for example_id in cfg['example_ids']:
        derived_seed=seed_for_example(cfg['seed'],example_id); derived_seeds.append({'example_id':example_id,'seed':derived_seed})
        text=build_chat_prompt(tok,ds[example_id]['question'],cfg['system_instruction']); enc={k:v.cuda() for k,v in tokenize_prompt(tok,text).items()}
        ...
        err=(check-out.latent_states.float().cpu()).abs().max().item(); state_errors.append(err); validated.append(err<1e-4)
    ...
    with output.open('xb') as stream:
        np.savez_compressed(stream,h0=h0,x=x,trajectory=traj,mask=mask,example_ids=np.array(cfg['example_ids']),input_lengths=np.array([len(i) for i in ids]))
    write_json_exclusive(output.with_suffix('.json'),{'config':cfg,'state_capture':'h0 is initialized state; h_d is after d core_block_forward applications, before final ln_f and coda','max_ln_f_state_error':max(state_errors),'all_state_validated':all(validated),'seed_protocol':SEED_PROTOCOL,'derived_seeds':derived_seeds})
