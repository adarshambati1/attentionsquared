...
from scripts.run_004_functional_overfit_gate_v5 import (
    CACHE_ROOT,
    INITIALIZATION,
    OUTPUT_ROOT,
...
ATTEMPTS_ROOT = Path("/workspace/functional_overfit_v5_native_attempts")
...
def matching_training_processes(proc_root: Path = Path("/proc")) -> list[dict[str, object]]:
    ...
    for process_dir in proc_root.glob("[0-9]*"):
        ...
        if not argv or not Path(argv[0]).name.startswith("python"):
            continue
        if not any(
            Path(argument).name == "run_004_functional_overfit_gate_v5.py"
