...
def main(config,cache,output,steps):
 ...
 for k in [1,2,4]:
  ...
  for step in range(1,steps+1):
   ...
   with torch.autocast(device_type='cuda',dtype=torch.bfloat16): z,_,_=a2(h0[None],x[None]); student=hug(input_ids=ids[None],input_states=z[:,15].to(torch.bfloat16),num_steps=0,use_cache=False).logits[:,s:e]; loss=torch.nn.functional.kl_div(torch.log_softmax(student.float(),-1),torch.softmax(tl,-1),reduction='none').sum(-1).mean()
   loss.backward(); torch.nn.utils.clip_grad_norm_(a2.parameters(),1.0); opt.step(); logs.append(loss.item())
   if step%100==0: print(f'functional-full K={k} step={step}/{steps} train_kl={np.mean(logs[-100:]):.6g} seconds={time.perf_counter()-start:.1f}',flush=True)
  result={'K':k,'objective':'full_answer_teacher_KL_only','steps':steps,'parameters':sum(p.numel() for p in a2.parameters()),'train_kl_final':float(logs[-1]),'seconds':time.perf_counter()-start}; kd=output/f'K{k}'; kd.mkdir(exist_ok=True); torch.save({'state_dict':a2.state_dict(),'config':{**c,'K':k,'objective':'full_answer_teacher_KL_only'},'result':result},kd/'model.pt'); (kd/'result.json').write_text(json.dumps(result,indent=2)); summary.append(result); del a2; torch.cuda.empty_cache()
