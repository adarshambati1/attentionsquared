...
def main(config,cache,output,steps):
 ...
 for k in [1,2,4]:
  ...
  for step in range(1,steps+1):
   ...
   with torch.autocast(device_type='cuda',dtype=torch.bfloat16): z,_,_=a2(h0[None],x[None]); student=hug(input_ids=ids[None],input_states=z[:,15].to(torch.bfloat16),num_steps=0,use_cache=False).logits[:,s:e]; loss=torch.nn.functional.kl_div(torch.log_softmax(student.float(),-1),torch.softmax(tl,-1),reduction='batchmean')
