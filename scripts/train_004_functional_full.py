...
def main(config,cache,output,min_steps,max_steps):
 ...
 for k in [1,2,4]:
  ...
  result={'K':k,'objective':'full_answer_teacher_KL_only','steps':step,'parameters':sum(p.numel() for p in a2.parameters()),'train_kl_token_final':float(logs[-1]),'best_val_kl_token':best_val,'teacher_self_kl':0.0,'seconds':time.perf_counter()-start}; kd=output/f'K{k}'; kd.mkdir(exist_ok=True); torch.save({'state_dict':a2.state_dict(),'config':{**c,'K':k,'objective':'full_answer_teacher_KL_only'},'result':result},kd/'model.pt'); (kd/'result.json').write_text(json.dumps(result,indent=2)); summary.append(result); del a2; torch.cuda.empty_cache()
 (output/'summary.json').write_text(json.dumps(summary,indent=2))
if __name__=='__main__':
 p=argparse.ArgumentParser(); p.add_argument('--config',type=Path,default=Path('configs/004b_fixed_k.json')); p.add_argument('--cache',type=Path,default=Path('results/004_functional/training_cache')); p.add_argument('--output',type=Path,default=Path('results/004_functional/models_full')); p.add_argument('--min-steps',type=int,default=2000); p.add_argument('--max-steps',type=int,default=5000); a=p.parse_args(); main(a.config,a.cache,a.output,a.min_steps,a.max_steps)
