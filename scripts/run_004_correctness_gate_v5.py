...
OUTPUT = Path("/workspace/functional_protocol/correctness_gate_cache_v5_native_v1.json")
CACHE_VALIDATION = ROOT / "results/004_functional/cache_v5_smoke_validation_v1.json"
EXPECTED_CACHE_MANIFEST_SHA256 = "55a5cc5cc77bf969ef200b49dae7d50d63ef97a88e608978902c6e50f5040583"
EXPECTED_CACHE_VALIDATION_SHA256 = "65dd76fca9cba8df435b52c36f282ab8420fb742d8c4d65f61b813904e4af006"
MODEL_ID = "tomg-group-umd/huginn-0125"
MODEL_REVISION = "bb6621b65e90b6a4b9b29ef88dc83866d450470c"
PROTOCOL = "functional-correctness-gate-cache-v5-native-v1"
EXAMPLE_ID = 0
...
def run_gate(cache_root: Path, initialization: Path) -> dict:
    ...
    with torch.no_grad():
        teacher_logits = frozen_coda_logits_from_normalized_state(
            huginn, example["h16_teacher"], frequencies
        ).detach()
    ...
    with torch.no_grad(), torch.autocast(device_type="cuda", dtype=torch.bfloat16):
        ...
        baseline_logits = frozen_coda_logits_from_normalized_state(
            huginn, baseline_output[:, 15], frequencies
        )
...
def main(cache_root: Path, initialization: Path, output: Path) -> dict:
    ...
    if cache_manifest_sha256 != EXPECTED_CACHE_MANIFEST_SHA256:
        raise RuntimeError("cache-v5 native smoke manifest identity changed")
    if cache_validation_sha256 != EXPECTED_CACHE_VALIDATION_SHA256:
        raise RuntimeError("cache-v5 independent validation identity changed")
