#!/usr/bin/env python3
"""Independent production validation/replay for the Phase 9R cache-v3 smoke."""
...
CONFIG = Path("configs/004_functional_cache_v3_smoke.json")
...
def _load_locked_inputs() -> tuple[dict, list[dict]]:
    ...
    fixed = {
        "protocol": "functional-cache-v3-smoke-build-v1",
        "model_id": MODEL_ID,
...
def validate_file_set(root: Path = CACHE_ROOT) -> list[Path]:
    ...
    allowed = {root / ".functional-cache-v3.lock"}
...
def validate_static(root: Path = CACHE_ROOT):
    ...
    manifest_fixed = {
        "protocol": "functional-cache-v3-smoke-manifest-v1",
        ...
        "state_dtype": "float16",
        "hidden_size": 5280,
        "extraction_location": "explicit scheduled input_states -> frozen Huginn D16; x=post-prelude recurrent input; h16=returned normalized pre-coda latent",
        "example_ids": list(IDS),
        "sequence_count": 8,
    ...
    for example_id, (row, item) in enumerate(zip(rows, items)):
        ...
        string_expected = {
            ...
            "cache_protocol": "functional-cache-v3-smoke-item-v1",
        ...
        for key in ("h0","x","h16"):
            if value[key].shape != (len(raw), 5280) or value[key].dtype != np.float16 or not np.isfinite(value[key]).all():
                raise ValueError(f"cached state schema failed {key} ID={example_id}")
        values.append(value)
    ...
    try:
        for example_id, (row, cached) in enumerate(zip(rows, values)):
            ...
            for prefix_length in range(1, len(raw) + 1):
                ...
                observed_h0 = (
                    full_schedule[:, :prefix_length]
                    .float().cpu().numpy()[0].astype(np.float16)
                )
            ...
            for key in ("x","h16"):
                observed = teacher[key].float().cpu().numpy()[0].astype(np.float16)
                if observed.tobytes() != cached[key].tobytes():
                    raise ValueError(f"cached {key} fp16 replay is not byte-identical ID={example_id}")
            ...
            records.append({"example_id":example_id,"prompt_tokens":count,"sequence_tokens":len(raw),"initializer_calls":2,"rng_restored_each_request":True,"full_bf16_schedule_sha256":full_hash,"schedule_overlap_bitwise":True,"cached_h0_fp16_exact":True,"d16_calls_each_request":16,"cached_x_h16_fp16_byte_exact":True,"normal_vs_decomposed_logits_exact":True,"diagnostics":diagnostics})
            del prompt_schedule, full_schedule, teacher, prefix, full_ids, prompt_tensor
            torch.cuda.empty_cache()
    finally:
        model.initialize_state = original_initializer
    return {"examples":records,"diagnostics_global":{"represented_prompt_tokens":total_tokens,"globally_token_normalized_kl":total_kl/total_tokens,"top1_agreement":total_top1/total_tokens},"diagnostics_only_no_acceptance_threshold":True}
