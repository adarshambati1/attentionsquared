...
def losses(wrapper,ids,h0,answer_start,c):
 out=wrapper(ids,h0,compute_fixed_point=True);task,count=answer_cross_entropy(out.logits,ids,answer_start);fixed=fp_loss(out,answer_start,count,c['fixed_point_epsilon']);return out,task,fixed,task+c['fixed_point_weight']*fixed,count
def validate(wrapper,huginn,tok,ds,ids,c):
 ...
 return {'answer_token_cross_entropy':task_sum/tokens,'fixed_point_relative_residual':fp_sum/tokens,'total_objective':task_sum/tokens+c['fixed_point_weight']*fp_sum/tokens,'answer_tokens':tokens}
def payload(wrapper,opt,step,val,config_path,initialization_sha):
 return {'protocol':'functional-fixed-point-jump-checkpoint-v1','step':step,'validation':val,'predictor_state_dict':{k:v.detach().cpu() for k,v in wrapper.predictor.state_dict().items()},'optimizer_state_dict':opt.state_dict(),'git_commit':git_commit(),'config_sha256':hashlib.sha256(config_path.read_bytes()).hexdigest(),'predictor_initialization_sha256':initialization_sha}
def main():
