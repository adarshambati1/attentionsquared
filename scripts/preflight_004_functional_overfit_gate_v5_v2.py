#!/usr/bin/env python3
"""Record execution-host evidence before corrected Phase 11 native-v5 training."""
...
from scripts.run_004_functional_overfit_gate_v5_v2 import (
    CACHE_ROOT,
    INITIALIZATION,
    OUTPUT_ROOT,
...
ATTEMPTS_ROOT = Path("/workspace/functional_overfit_v5_native_v2_attempts")
...
def matching_training_processes(proc_root: Path = Path("/proc")) -> list[dict[str, object]]:
    ...
    for process_dir in proc_root.glob("[0-9]*"):
        ...
        if not argv or not Path(argv[0]).name.startswith("python"):
            continue
        if not any(
            Path(argument).name == "run_004_functional_overfit_gate_v5_v2.py"
